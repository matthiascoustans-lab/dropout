...
const DEFAULT_FEED = [
  { id:1, type:"VIDEO", source:"Chargement...", title:"DropOut", italic:"charge vos ressources", excerpt:"Claude analyse votre passion et centralise les meilleures ressources en temps réel.", duration:"...", bg:"linear-gradient(135deg,#0a0005,#1a0008,#080808)" },
];
...
export default function Home() {
  ...
  const [chatMsgs, setChatMsgs] = useState([{role:"ai", content:"Bienvenue. Dis-moi ce que tu viens de voir ou découvrir sur la Porsche 911 — je t'ouvre un univers."}]);
  ...
  const sendChat = async (text?: string) => {
    ...
    try {
      ...
      setChatMsgs(p => [...p, {role:"ai", content:"Erreur de connexion."}]);
    }
    setChatLoad(false);
  };
  ...
  return (
    <div style={{display:"grid", gridTemplateColumns:"56px 1fr 300px", height:"100vh", overflow:"hidden", background:"#080808", color:"#f0ede8", fontFamily:"'DM Sans',sans-serif"}}>
      ...
      {/* FEED */}
      <div style={{position:"relative", height:"100vh", overflow:"hidden"}} onWheel={onWheel} onTouchStart={e => { touchStart.current = e.touches[0].clientY; }} onTouchEnd={e => { const d = touchStart.current - e.changedTouches[0].clientY; if (Math.abs(d) > 50) { if (d > 0) goTo(current+1); else goTo(current-1); }}}>
        ...
        <div style={{position:"absolute", top:0, left:0, right:0, height:52, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 24px", background:"linear-gradient(to bottom,#080808,transparent)", zIndex:20}}>
          ...
          <div style={{fontSize:10, color:"#444", letterSpacing:"1px"}}>{loading ? "Loading..." : `${current+1} / ${feed.length}`}</div>
        </div>
        ...
        <div style={{position:"relative", height:"100%", width:"100%"}}>
          {loading ? (
            ...
            <div key={card.id} style={cardStyle(getState(idx), card.bg || "linear-gradient(135deg,#0a0005,#1a0008,#080808)")}>
      ...
      <div style={{background:"#0f0f0f", borderLeft:"1px solid rgba(255,255,255,0.05)", display:"flex", flexDirection:"column", height:"100vh", overflow:"hidden"}}>
        <div style={{padding:"18px 18px 14px", borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
          ...
          <div style={{fontSize:10, color:"#444", letterSpacing:"1px"}}>Powered by Claude · {feed.length} resources loaded</div>
        </div>
              ...
              <div style={{flex:1, display:"flex", flexDirection:"column", gap:12, paddingBottom:8}}>
                {chatMsgs.map((m, i) => (
                  <div key={i} style={{display:"flex", gap:8, flexDirection: m.role==="user" ? "row-reverse" : "row"}}>
                    <div style={{width:22, height:22, background: m.role==="ai" ? "#c0001a" : "#1e1e1e", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:700, color:"white", flexShrink:0, fontFamily:"'Bebas Neue',cursive", letterSpacing:"1px"}}>{m.role==="ai" ? "DO" : "M"}</div>
                    <div style={{padding:"9px 12px", fontSize:12, lineHeight:1.65, maxWidth:"80%", fontWeight:300, background: m.role==="ai" ? "#161616" : "rgba(192,0,26,0.08)", color: m.role==="ai" ? "#b8b4ae" : "#f0ede8", borderLeft: m.role==="ai" ? "2px solid #c0001a" : "none", border: m.role==="user" ? "1px solid rgba(192,0,26,0.2)" : undefined, whiteSpace:"pre-wrap" as const}}>{m.content}</div>
                  </div>
                ))}
                {chatLoad && (
                  <div style={{display:"flex", gap:8}}>
                    <div style={{width:22, height:22, background:"#c0001a", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:700, color:"white", fontFamily:"'Bebas Neue',cursive"}}>DO</div>
                    <div style={{padding:"9px 12px", background:"#161616", borderLeft:"2px solid #c0001a"}}>
        ...
        {rpTab==="chat" && (
          <div style={{padding:"12px", borderTop:"1px solid rgba(255,255,255,0.05)"}}>
            <div style={{display:"flex", gap:8}}>
              <input value={chatIn} onChange={e => setChatIn(e.target.value)} onKeyDown={e => { if(e.key==="Enter") sendChat(); }} placeholder="J'ai vu Le Mans 66..." style={{flex:1, background:"#161616", border:"1px solid rgba(255,255,255,0.09)", padding:"9px 12px", fontSize:12, fontFamily:"'DM Sans',sans-serif", color:"#f0ede8", outline:"none"}}/>
              <button onClick={() => sendChat()} disabled={chatLoad || !chatIn.trim()} style={{width:34, height:34, background:"#c0001a", border:"none", color:"white", fontSize:14, display:"flex", alignItems:"center", justifyContent:"center"}}>→</button>
            </div>
          </div>
        )}
        ...
        <div style={{padding:"12px", borderTop:"1px solid rgba(255,255,255,0.05)"}}>
          ...
          <div style={{display:"flex", gap:5, flexWrap:"wrap" as const}}>
            {PASSIONS.map(p => (
              <button key={p} onClick={() => switchPassion(p)} style={{padding:"5px 10px", background: activePassion===p ? "rgba(192,0,26,0.06)" : "#161616", border: activePassion===p ? "1px solid rgba(192,0,26,0.3)" : "1px solid rgba(255,255,255,0.06)", fontSize:10, color: activePassion===p ? "#c0001a" : "#555", fontFamily:"'DM Sans',sans-serif", letterSpacing:".5px"}}>{p}</button>
            ))}
